...
// The Goodreads cover gate this file used to pin on the cached-reuse path is
// deleted (`docs/design-subtitle-matching.md` r3, C2). No title comparison stands
// between a Goodreads payload and the cover field on either path now, so there is
// no shared gating policy left to assert. The language-routing policy above is
// still enforced at the same chokepoint and still covered.
