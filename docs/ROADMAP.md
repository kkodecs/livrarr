...
| Status | Area | # | Item |
|:------:|------|---|------|
| ⬜ | Perf | — | **E-prerequisite — #144 remainder (ID-assignment standardization).** Found 2026-06-12 during Sprint C testing: (a) single-anchor adds starve the scatter — a GR-link add landed `failed` with every provider `skipped_no_anchor` (stopgap shipped: add-door anchor-completion ladder in `run_unified_enrichment`, gated on zero ol/isbn/asin/hc — subsume into one "complete anchors whenever the scatter would starve" rule at the decision gate, replacing the stopgap + the refresh ladder copy); (b) **Pending-seeded doors (series monitor worker, Readarr import) wait for a background convergence job that no longer exists** (`enrichment_retry_tick` removed; `retry_all_incomplete` is user-triggered only; the worker's "background resolver converges it" comment is aspirational) — PO decision needed: restore a background tick, or make manual-refresh semantics explicit |
| ⬜ | Perf | — | **Enrichment-scatter parallelization.** Principled cut (PO, 2026-06-10): identity phase resolves all anchors incl. ASIN (depends on the #144 remainder row above), then one join across providers, per-leg timeout/rate-bucket/breaker unchanged, merge untouched. Projected: ~2.2 s → ~1 s per work; bulk 8 min → ~2.5 min |
