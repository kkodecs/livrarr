    ...
    Ok(Grab {
        ...
        media_type: row
            ...
            .and_then(|s| match s.as_str() {
                "ebook" => Some(librarr_domain::MediaType::Ebook),
                "audiobook" => Some(librarr_domain::MediaType::Audiobook),
                _ => None,
            }),
...
impl GrabDb for SqliteDb {
    ...
    async fn upsert_grab(&self, req: CreateGrabDbRequest) -> Result<Grab, DbError> {
        ...
        .bind(req.media_type.map(|mt| match mt {
            librarr_domain::MediaType::Ebook => "ebook",
            librarr_domain::MediaType::Audiobook => "audiobook",
        }))
        .bind(&now)
        .execute(self.pool())
        .await
