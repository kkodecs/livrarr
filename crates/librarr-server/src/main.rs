...
use librarr_server::config::{AppConfig, LogLevel};
use librarr_server::router::build_router;
use librarr_server::state::AppState;

/// Librarr — self-hosted ebook and audiobook library manager.
#[derive(Parser)]
#[command(name = "librarr", version)]
struct Cli {
...
async fn main() {
    ...
    info!("Librarr starting — data directory: {}", data_dir.display());

    // Step 4: Connect to SQLite.
    let pool = match librarr_db::pool::create_sqlite_pool(&data_dir).await {
        Ok(p) => p,
        Err(e) => {
    ...
    if let Err(e) = librarr_db::pool::run_migrations(&pool).await {
    ...
    let db = librarr_db::sqlite::SqliteDb::new(pool);
    let auth_service = Arc::new(librarr_server::auth_service::ServerAuthService::new(
        db.clone(),
    ));
    let http_client = librarr_http::HttpClient::builder()
        .timeout(std::time::Duration::from_secs(30))
        .user_agent(&format!("Librarr/{}", env!("CARGO_PKG_VERSION")))
        .build()
        .expect("failed to build HTTP client");
    let job_runner = librarr_server::jobs::JobRunner::new();
    let state = AppState {
        db,
        auth_service,
        http_client,
    ...
    librarr_server::jobs::recover_interrupted_state(&state).await;
...
fn load_config(data_dir: &std::path::Path) -> Result<AppConfig, String> {
    ...
    let config: AppConfig = if config_path.exists() {
        ...
        if raw.trim().is_empty() {
            ...
            if let Ok(val) = raw.parse::<toml::Value>() {
                librarr_server::config::warn_unknown_keys(&val);
            }
    ...
    librarr_server::config::validate_config(&config).map_err(|e| e.to_string())?;
    Ok(config)
}

fn init_tracing(log: &librarr_server::config::LogConfig) {
    ...
    let filter = EnvFilter::try_new(format!("librarr={level},tower_http={level}"))
