...
use librarr_domain::{EventType, Indexer};
...
async fn fetch_and_extract_hash(
    ...
    if download_url.starts_with("magnet:") {
        return librarr_download::extract_torrent_hash(&librarr_download::TorrentSource::Magnet(
    ...
    if let Ok(text) = std::str::from_utf8(&bytes) {
        if text.trim().starts_with("magnet:") {
            return librarr_download::extract_torrent_hash(
                &librarr_download::TorrentSource::Magnet(text.trim().to_string()),
            )
            .ok();
        }
    }
    ...
    librarr_download::extract_torrent_hash(&librarr_download::TorrentSource::TorrentFile {
...
pub(crate) fn qbit_base_url(client: &librarr_domain::DownloadClient) -> String {
