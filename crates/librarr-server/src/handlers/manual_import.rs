...
use librarr_db::{ConfigDb, LibraryItemDb, RootFolderDb, WorkDb};
use librarr_domain::{classify_file, normalize_for_matching, MediaType};
...
async fn import_single_item(
    ...
    user_id: i64,
    item: &ImportItem,
    existing_works: &[librarr_domain::Work],
    root_folders: &[librarr_domain::RootFolder],
    ...
    if target.exists() {
        ...
        match state
            .db
            .create_library_item(librarr_db::CreateLibraryItemDbRequest {
                user_id,
                work_id,
...
fn find_existing_work<'a>(
    works: &'a [librarr_domain::Work],
    ol_key: &str,
    title: &str,
    author: &str,
) -> Option<&'a librarr_domain::Work> {
    works
        .iter()
...
async fn llm_parse_filenames(
    ...
    let cfg: librarr_db::MetadataConfig = match state.db.get_metadata_config().await {
        Ok(c) => c,
