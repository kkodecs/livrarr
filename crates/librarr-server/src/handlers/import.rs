...
use librarr_domain::EventType;
use librarr_domain::{classify_file, sanitize_path_component};
use librarr_tagwrite::TagWriteStatus;
...
pub async fn import_grab(
    ...
    let has_enrichment = work.enrichment_status != librarr_domain::EnrichmentStatus::Pending;
...
pub async fn import_single_file(
    ...
    user_id: i64,
    work_id: i64,
    tag_metadata: Option<&librarr_tagwrite::TagMetadata>,
    cover: Option<&[u8]>,
    media_mgmt: &librarr_db::MediaManagementConfig,
    author_name: &str,
    title: &str,
    ...
    if let Some(metadata) = tag_metadata {
        ...
        match librarr_tagwrite::write_tags(
            tmp_path.clone(),
            metadata.clone(),
...
async fn import_mp3_batch(
    ...
    if let Some(metadata) = tag_metadata {
        match librarr_tagwrite::write_tags_batch(
            tmp_paths.clone(),
            metadata.clone(),
            cover.map(|c| c.to_vec()),
        )
...
fn filter_preferred_formats(
    ...
    config: &librarr_db::MediaManagementConfig,
) -> Vec<SourceFile> {
...
pub async fn fetch_qbit_content_path(
    state: &AppState,
    client: &librarr_domain::DownloadClient,
    hash: &str,
...
pub fn build_tag_metadata(work: &librarr_domain::Work) -> librarr_tagwrite::TagMetadata {
    librarr_tagwrite::TagMetadata {
...
pub async fn retag_library_items(
    state: &AppState,
    work: &librarr_domain::Work,
    items: &[librarr_domain::LibraryItem],
) -> Vec<String> {
