...
use librarr_domain::{EventType, NotificationType};
                ...
                "Failed" => {
                    ...
                    if let Err(e) = state
                        ...
                        .update_grab_status(
                            grab.user_id,
                            grab.id,
                            librarr_domain::GrabStatus::Failed,
                            Some(fail_msg),
                        )
                        .await
                    {
...
async fn enrichment_retry_tick(state: AppState, _cancel: CancellationToken) -> Result<(), String> {
    ...
    for work in &works {
        ...
        {
            Ok(updated) => {
                if updated.enrichment_status == librarr_domain::EnrichmentStatus::Enriched
                    || updated.enrichment_status == librarr_domain::EnrichmentStatus::Partial
                {
