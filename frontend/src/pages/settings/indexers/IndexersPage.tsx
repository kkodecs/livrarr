      ...
      <PageContent className="space-y-4">
        ...
        {isAdmin && (
          <ProwlarrImportSection
            onImported={() => qc.invalidateQueries({ queryKey: ["indexers"] })}
          />
        )}
...
function ProwlarrImportSection({ onImported }: { onImported: () => void }) {
  ...
  // Always fetch config so we can show status and pre-fill.
  ...
  const importMutation = useMutation({
    mutationFn: api.importIndexersFromProwlarr,
    onSuccess: (data) => {
      setResult(data);
      if (data.imported > 0) {
        toast.success(`Imported ${data.imported} indexer${data.imported !== 1 ? "s" : ""}`);
        onImported();
      } else if (data.skipped > 0) {
        toast.info("All indexers already exist — nothing to import");
      }
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <section data-tour="prowlarr-import" className="rounded-lg border border-border bg-zinc-900/50">
      <button
      ...
      {open && (
        <div className="p-5 space-y-4 max-w-xl">
          <p className="text-sm text-muted">
            Import indexers from your Prowlarr instance. Duplicates (matching URL) will be skipped.
          </p>
          ...
          <button
            ...
            className="rounded bg-brand px-4 py-2 text-sm font-medium text-white hover:bg-brand-hover disabled:opacity-50"
          >
            {importMutation.isPending ? "Importing..." : "Import Indexers"}
          </button>
        </div>
      )}
    </section>
