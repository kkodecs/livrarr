...
## 2026-08-20 — identity-layer-rewrite close-out fold (rounds 7-22 complete)

- Note for the record: insight 97 (REQ-027 v11 per-provider search eligibility + the GR mint-direction rule + the provenance-aware taxonomy heal, round 21) was seat-written during rounds 21/22 and committed in `ba99290e` without a log entry; this entry covers it.
- insights.md: added insight 98 (round 7 — leading articles identity-irrelevant per PO ruling 2026-08-16; ONE article authority `strip_leading_identity_article`/`IDENTITY_ARTICLES` shared by matcher, stored v2 main, and `identity_key_flat`; `identity_title_policy_generation=2` heal semantics; the refresh cover-gate one-line-per-pass observability rule; search-card covers are `coverManual=false`).
- insights.md: added insight 99 (rounds 10-12 — the three self-feeding settlement wires and the zero-caller designed persist seam; chase accounting burns only on a real spawned fetch, `identity_round11_attempt_reheal`; machine continuations carry `EnrichmentPass` provenance through `apply_add_background_handoff`; generation-bound handoffs fail closed; round-12 test-isolation lessons). Flagged residual: round 10's fileless MachineAlone door-matrix amendment was never spec-folded; moot on Readarr-door elimination, precedent durable.
- insights.md: amended insight 63 (GR-cover containment: `GOODREADS_COVER_CANDIDATES_ENABLED=false` at the one `cover_resolution.rs` seam, rank tables unchanged; 51+44 wrong covers re-selected by one-shot heal; re-enable owned by the GR feature) and insight 96 (the 2026-08-18 "GR layout drift broke cover-image extraction" diagnosis REVERSED — wrong covers were the namespace conflation; the two live-measured soft-block classes: empty-200 zero-byte bodies with ten banked captures, ~59KB Next-shell bodies).
- integrations/goodreads.md: added the mint-direction rules (only autocomplete `workId` and the Book page's Work-entity `legacyId` mint `GoodreadsWork`; file/list/legacy/direct ids mint `GoodreadsBookEdition`; provenance-aware heal) and a "Soft-block response classes" section (empty-200 + Next-shell, detection owned by the GR feature).
- Corrected the reversed diagnosis at its two remaining sites outside the wiki: `spec-identity-layer-rewrite.md` (v9 changelog line annotated; the Covers containment note rewritten with the true causes — rationale only, no requirement changed) and the `cover_resolution.rs` containment comment (now states the namespace cause and the anti-bot-detection re-enable condition instead of "drifted parser").
- index.md insight count 96→99. Verified no wiki page carries the reversed drift claim (repo-wide scan; the claim lived only in the spec, the code comment, and session artifacts).

## 2026-08-22 — identity-lifecycle-audit close-out fold
